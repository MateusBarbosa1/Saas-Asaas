...
function slugify(text) {
  return (
    text.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'novo-site'
  );
}
...
export default function NewSiteWizard({ mode = 'add-site', onComplete }) {
  ...
  function submit() {
    ...
    const site = {
      id: `${slugify(label)}-${Date.now().toString(36).slice(-4)}`,
      ...
      estimatedDelivery: '',
      pages: estrutura.map((name) => ({ name, done: false })),
      plan: {
        name: 'A definir',
        price: 'A combinar',
        cycle: form.orcamento ? `Orçamento informado: ${form.orcamento}` : 'A definir com a equipe',
        nextPayment: '—',
        includes: [],
      },
      updates: [{
        date: new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }),
        text: 'Briefing recebido, aguardando início do estudo de mercado.',
      }],
      files: [],
    };

    const account = mode === 'signup'
      ? { nome: form.nomeConta, empresa: form.empresa, email: form.emailConta }
      : null;

    onComplete(site, account);
  }
    ...
    <>
      ...
      <div className="card briefing-body">
        ...
        <div className="briefing-nav">
          ...
          <button className="btn btn-brass" onClick={next} disabled={!canAdvance}>
            {isLast ? (mode === 'signup' ? 'Criar minha conta' : 'Enviar pedido de novo site') : 'Continuar'}
